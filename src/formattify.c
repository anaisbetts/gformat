...
static spawn_cb(GPid pid, gint status, gpointer data)
{
	...
	g_io_channel_read_to_end(out, &cb_data->out_fd, &dontcare, NULL);
	g_io_channel_read_to_end(err, &cb_data->out_fd, &dontcare, NULL);
	...
	g_idle_add(data->real_cb, cb_data);
	g_free(data);
}

gboolean spawn_async_get_output(gchar** argv, GFunc callback, gpointer user_data)
{
	GPid pid;
	gint out_fd;
	gint error_fd;
	...
	g_child_watch_add(pid, spawn_cb, user_data);
}
